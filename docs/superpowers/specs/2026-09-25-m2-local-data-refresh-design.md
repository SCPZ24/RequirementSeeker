...
1. Preflight: verify exact source and destination paths, manifest hash, source/readability, no path redirects, enough free space, no `sanitized-v2/` or `labels-v2/` target or corresponding `.backup`/staging transaction, and no human edits in the old label tree. Record aggregate counts and content-tree hashes for `raw/`, `sanitized/`, and `labels/` without displaying identifiers or text. Abort on any mismatch or unexpected target.
2. Generate `sanitized-v2/` once from `raw/` and the approved manifest using `rs-dataset sanitize` and the stable HMAC environment name. The current sanitizer stages output and publishes it atomically. Never direct it at the existing `sanitized/`; a successful in-place run would delete its temporary backup.
